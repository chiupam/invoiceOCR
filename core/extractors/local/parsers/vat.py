...
# Max horizontal distance (pt) from label to value when looking for a name
# value. Beyond this we treat the candidate as belonging to the other side.
_NAME_VALUE_MAX_X_DIST = 200.0
...
class VatParser(Parser):
    ...
    def _extract_parties(blocks: list[TextBlock], parsed: ParsedInvoice) -> None:
        ...
        Three VAT layout shapes are handled:
          1. Merged: '名称：中国联合网络通信有限公司北京市分公司' (whole text in one block)
          2. Old-style: '名' + '称:'  label pairs (split blocks, colon in 称)
          3. Number-of-purchases: '名' + '称'  (split blocks, NO colon — JD)
        """
        # Names: blocks containing 名称 label (forms the merged case)
        name_blocks = []  # list of (block, value_or_empty)
        for b in blocks:
            t = b.text.strip()
        ...
        # Old-style layout: 名 and 称 (with or without colon) are SEPARATE blocks.
        # Match: '名' followed by '称' or '称:' on the same row, then find the value
        # block to the right. We accept both with-colon (e.g. 出行 old-style) and
        # no-colon (JD 数电发票 space-split) variants.
        if not name_blocks:
            for i, b in enumerate(blocks):
                if b.text.strip() != "名":
                    continue
                # Find 称 nearby (same row). Accept either form (with or without colon).
                for nb in blocks:
                    if nb is b or not nb.text.strip().startswith("称"):
                        continue
                    if abs(nb.bbox[1] - b.bbox[1]) <= 5.0:
                        # Use the 名 block as the label anchor (it's the
                        # visually canonical label). The value is in a block
                        # to the right of 称, at a similar Y.
                        name_blocks.append((b, ""))
                        break

        # Sort by X — leftmost = buyer, rightmost = seller.
        # If only one label block (e.g. JD has no buyer name), sort by page
        # midpoint to decide the slot.
        name_blocks.sort(key=lambda x: x[0].bbox[0])

        # First pass: assign values that were already extracted (merged case).
        # Reference point: with sorted name_blocks, leftmost → buyer, rightmost → seller.
        # With one label-block, decide buyer/seller by its X position vs page midpoint.
        if name_blocks:
            page_w = max((nb.bbox[2] for nb in blocks), default=600)
            for b, value in name_blocks:
                if not value:
                    continue
                if len(name_blocks) == 1:
                    slot = 'buyer' if b.bbox[0] < page_w / 2 else 'seller'
                else:
                    slot = 'buyer' if b is name_blocks[0][0] else 'seller'
                if slot == 'buyer' and parsed.buyer_name == '':
                    parsed.buyer_name = value
                elif slot == 'seller' and parsed.seller_name == '':
                    parsed.seller_name = value

        # Second pass: for label-only blocks, find the value to the right.
        # Cap X-distance to avoid cross-side grabbing (e.g. JD: left label
        # claiming the right-side company name) and skip the label-partner block.
        for b, value in name_blocks:
            if value:
                continue
            best = None
            ...
            for nb in blocks:
                ...
                if "方" in nb.text and len(nb.text) <= 2:
                    continue
                if nb.text.strip() in ("名", "称"):
                    continue
                if nb.text.strip().startswith("称") and len(nb.text.strip()) <= 3:
                    continue  # skip 称/:名-partner block
                dy = abs(nb.bbox[1] - b.bbox[1])
                dx = nb.bbox[0] - b.bbox[0]
                if dy <= 8.0 and 0 < dx <= _NAME_VALUE_MAX_X_DIST:
                    if dy < best_dist:
                        best = nb
                        best_dist = dy
            if best is None:
                # Fallback: relax Y to 10pt, still X-capped
                for nb in blocks:
                    if nb is b or not nb.text.strip():
                        continue
                    if nb.text.strip().startswith("统一"):
                    ...
                    if nb.text.strip().startswith("称") and len(nb.text.strip()) <= 3:
                        continue
                    dx = nb.bbox[0] - b.bbox[0]
                    if 0 < dx <= _NAME_VALUE_MAX_X_DIST and abs(nb.bbox[1] - b.bbox[1]) <= 10.0:
                        best = nb
                        break
            if best is None:
                continue
            val = best.text.strip()
            # Decide slot by this label's X position.
            # With one label-block, use page midpoint. With two, leftmost = buyer.
            if len(name_blocks) == 1:
                page_w = max((nb.bbox[2] for nb in blocks), default=600)
                this_slot = 'buyer' if b.bbox[0] < page_w / 2 else 'seller'
            else:
                this_slot = 'buyer' if b is name_blocks[0][0] else 'seller'
            if this_slot == 'buyer' and parsed.buyer_name == '':
                parsed.buyer_name = val
            elif this_slot == 'seller' and parsed.seller_name == '':
                parsed.seller_name = val

        # Tax IDs from 统一社会信用代码/纳税人识别号 labels.
        # Layouts:
        #  - 数电发票: two label blocks side-by-side (left buyer, right seller).
        #    The right one usually embeds the value (e.g. "统一社会信用代码/纳税人识别号:91110302MA01XXXXX");
        #    the left may be empty (个人 has no code) or have a separate value block.
        #  - Old-style (2023): plain "纳税人识别号:" labels in top (buyer) and
        #    bottom (seller) blocks.
        # Robust strategy: collect all (x, value) pairs where the block has
        # BOTH the label AND a value, plus any (x, value) pair on the SAME
        # ROW as a label block. Sort by X, assign leftmost to buyer and
        # rightmost to seller. If only one found, default to seller (B2B safe).
        ...
        for b in blocks:
            ...
            # 2. Value on a nearby block at similar Y
            if not m or not m.group(1):
                for nb in blocks:
                    ...
                    if vm:
                        ...
                        break
        # Dedupe by value (same tax ID may be matched by multiple left-side labels)
        seen = set()
        unique_pairs = []
        for x, v in tax_pairs:
            if v not in seen:
                seen.add(v)
                unique_pairs.append((x, v))
        tax_pairs = unique_pairs
    ...
    def _extract_items(blocks: list[TextBlock]) -> list[dict]:
        ...
        A VAT item row is: 名称 ... 金额 税率% 税额.
        The 税率% word is the marker:
          - tax_rate: the % word ("13%", "3%")
          - amount:   decimal just LEFT of the % word
          - tax:      first decimal RIGHT of the % word
        """
        lines: dict[int, list] = {}
        for b in blocks:
            y = int(b.bbox[1] / 4.0)
