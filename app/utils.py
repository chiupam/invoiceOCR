...
# 添加用于识别发票二维码的函数
def scan_invoice_qrcode(image_path):
    """
    扫描发票图片上的二维码并解析内容
    ...
    try:
        ...
        if image is None:
            current_app.logger.error(f"无法打开图片: {image_path}")
            return None
            
        # 裁剪图片左上角区域(通常二维码位于左上角)
        height, width = image.shape[:2]
        # 左上角区域 - 大约占图片的1/3区域
        qr_area = image[0:int(height/3), 0:int(width/3)]
        ...
        if not decoded_objects:
            current_app.logger.info(f"左上角未识别到二维码，尝试全图扫描")
            decoded_objects = decode(image)
            
        if not decoded_objects:
            current_app.logger.warning(f"未在图片中识别到二维码: {image_path}")
            return None
            
        # 获取第一个识别到的二维码内容
        qrcode_data = decoded_objects[0].data.decode('utf-8')
        current_app.logger.info(f"识别到二维码内容: {qrcode_data}")
        
        # 解析发票二维码内容
        # 标准规格的增值税发票二维码格式通常为：
        # 发票代码,发票号码,开票日期,金额,税额,校验码,等
        
        # 增值税发票二维码格式解析
        fields = qrcode_data.split(',')
        if len(fields) < 5:
            current_app.logger.warning(f"二维码内容格式不符合预期: {qrcode_data}")
            return {'raw_data': qrcode_data}
            
        # 解析字段
        qr_result = {
            'raw_data': qrcode_data,
            'invoice_code': fields[0] if len(fields) > 0 else '',
            'invoice_number': fields[1] if len(fields) > 1 else '',
            'invoice_date': None,
            'amount': fields[3] if len(fields) > 3 else '',
            'tax': fields[4] if len(fields) > 4 else '',
            'total_amount': None,
            'check_code': fields[5] if len(fields) > 5 else ''
        }
        ...
        if len(fields) > 2 and fields[2]:
            ...
            try:
                ...
                if re.match(r'^\d{8}$', date_str):
                    qr_result['invoice_date'] = datetime.strptime(date_str, '%Y%m%d').strftime('%Y-%m-%d')
            except Exception as e:
                current_app.logger.error(f"日期解析失败: {date_str}, 错误: {str(e)}")
                
        # 计算价税合计
        try:
            if qr_result['amount'] and qr_result['tax']:
                amount = float(qr_result['amount'])
                tax = float(qr_result['tax'])
                qr_result['total_amount'] = format(amount + tax, '.2f')
        except Exception as e:
            current_app.logger.error(f"金额计算失败: {qr_result['amount']}, {qr_result['tax']}, 错误: {str(e)}")
            
        return qr_result
        
    except ImportError as e:
        current_app.logger.error(f"缺少必要的库: {str(e)}，请安装pyzbar和opencv-python")
        return None
    except Exception as e:
        current_app.logger.error(f"二维码扫描过程中出错: {str(e)}")
        return None 